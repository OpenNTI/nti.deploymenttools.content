...
logger = logging.getLogger('nti_copy_course')
...
def _update_course_archive( course_archive, provider_id, start_date, end_date ):
    ...
    modified_course_archive = modified_course_archive[0] + '_modified' + modified_course_archive[1]
    temp_dir = mkdtemp()
    ...
    try:
        ...
        with open(os.path.join(temp_dir, 'course_info.json'), 'r') as fp:
            ...
            if start_date:
                course_info['startDate'] = datetime_isoformat(parse_datetime(start_date))
            if end_date:
                course_info['endDate'] = datetime_isoformat(parse_datetime(end_date))
...
def copy_course(course_ntiid, source_host, dest_host, username, site_library, admin_level, provider_id=None, start_date=None, end_date=None, cleanup=True):
    course_archive = None
    content_archives = []
    working_dir = mkdtemp()
    cwd = os.getcwd()
    try:
        ...
        course_archive = export_course( course_ntiid, source_host, username, password, UA_STRING )
        if source_host != dest_host:
            for content_package in _get_content_packages( course_archive ):
                ...
                content_archive = download_rendered_content( content_package, source_host, username, password, UA_STRING )
            ...
            for content_archive in content_archives:
                ...
                upload_rendered_content( content_archive[1], dest_host, username, password, site_library, UA_STRING )

        # Update course metadata with supplied information
        provider_id = provider_id or _get_provider_id( course_archive )
        course_archive = _update_course_archive( course_archive, provider_id, start_date, end_date )
...
def _parse_args():
    ...
    arg_parser.add_argument( '-a', '--admin-level', dest='admin_level', default='DefaultAPICopied',
