...
logger = logging.getLogger('nti_copy_content_package')
logging.captureWarnings(True)

UA_STRING = 'NextThought Content Package Copy Utility'

def copy_content_package(content_ntiid, source_host, dest_host, username, site_library, cleanup=True):
    ...
    try:
        ...
        content_archive = download_rendered_content( content_ntiid, source_host, username, password, UA_STRING )
        ...
        # Clean-up
        if cleanup:
            if content_archive and os.path.exists( content_archive ):
