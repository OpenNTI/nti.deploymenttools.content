...
def render_content(content_path, host, username, password, site_library, cleanup=True):
    ...
    content_archive = os.path.join(os.path.dirname(content_path), filename)
    content_archive = os.path.abspath(content_archive)

    try:
        logger.info('Rendering %s' % os.path.basename(content_path))

        # Render the content
        render(os.path.basename(content_path),
               out_format='xhtml', 
               nochecking=False)
        ...
        logger.info('Uploading render of %s to %s', 
                    content_name, host)

        content = upload_rendered_content(content_archive, 
                                          host, 
                                          username, 
                                          password, 
                                          site_library, 
...
def _parse_args(args=None):
    ...
    arg_parser.add_argument('contentpath', 
                            help="Content driver file to render")
...
def main(args=None):
    ...
    msg = 'Password for %s@%s: ' % (args.user, args.host)
    password = getpass.getpass(msg)

    render_content(content_path, args.host, args.user, password,
                   site_library, cleanup=args.no_cleanup)
